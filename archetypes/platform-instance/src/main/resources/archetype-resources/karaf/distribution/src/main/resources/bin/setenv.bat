...
rem https://github.com/openhab/openhab-distro/blob/master/distributions/openhab/src/main/resources/bin/setenv.bat

SET KARAF_TITLE=BaSys 4.0 Platform


:: set ports for HTTP(S) server
:check_http_port
IF NOT [%BASYS_HTTP_PORT%] == [] GOTO :http_port_set
set HTTP_PORT=8080
goto :http_port_done

:http_port_set
set HTTP_PORT=%BASYS_HTTP_PORT%
goto :http_port_done

:http_port_done

:check_https_port
IF NOT [%BASYS_HTTPS_PORT%] == [] GOTO :https_port_set
set HTTPS_PORT=8443
goto :https_port_done

:https_port_set
set HTTPS_PORT=%BASYS_HTTPS_PORT%
goto :https_port_done

:https_port_done

set EXTRA_JAVA_OPTS= ^
  -Dbasys.home=%BASYS_HOME% ^
  -Dbasys.conf=%BASYS_HOME%\config ^
