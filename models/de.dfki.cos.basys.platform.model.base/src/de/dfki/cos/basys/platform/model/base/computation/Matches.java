...
import de.dfki.cos.basys.platform.model.base.computation.MatchReport.MatchResult;
...
public class Matches {
	...
	static Logger logger = LoggerFactory.getLogger(Matches.class);
	...
	private static MatchReport _matchesPObject(EObject instance, PObject pattern) {
		...
		// logger.debug("[MATCHES] instance = " + instance + " pattern = " +
		// pattern);
		...
		if (!pattern.getType().getInstanceClass().isInstance(instance)) {
			...
			report.reason = String.format("%s is no instance of type %s.", instance.eClass().getName(), pattern.getType().getName());
			return report;
		}
		...
		for (EStructuralFeature feature : features) {
			...
			if (slot != null) {
				if (slot.getRange() != null && slot.getRange().size() > 0 && slot.getRange().get(0) instanceof PEmptySlot) {
				...
				if (feature instanceof EReference) {
					if (feature.isMany()) {
						...
						if (slot.getRange().size() > 0) {
							MatchReport tmp = _matchesPatterns((List<? extends EObject>) instance.eGet(feature), slot.getRange());
						...
						if (pobject.getQuantor() == PQuantorEnum.EXISTS || pobject.getQuantor() == PQuantorEnum.FOR_ALL) {
							if (tmp.result != MatchResult.MATCH_SUCCESS)
								return tmp;
							else {
								report.variables.putAll(tmp.variables);
	...
	private static MatchReport _matchesPatterns(List<? extends EObject> instances, List<PPattern> patterns) {
		...
		for (PPattern pattern : patterns)
			if (pattern.getQuantor() == PQuantorEnum.NOT_EXISTS) {
				if (pattern.getInstIdx() != -1) {
					try {
						logger.warn(String.format("Pattern %s with quantor NOT_EXISTS contains a value for InstIdx. This is ignored!", EmfPersistence.writeToString(pattern)));
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
		...
		for (PPattern pattern : patterns)
			if (pattern.getQuantor() == PQuantorEnum.FOR_ALL) {
				if (pattern.getInstIdx() != -1) {
					try {
						logger.warn(String.format("Pattern %s with quantor FOR_ALL contains a value for InstIdx. This is ignored!", EmfPersistence.writeToString(pattern)));
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
			...
			for (int i = 0; i < existsPatterns.size(); ++i) {
				...
				if (pattern.getInstIdx() != -1) {
					if (matchTable[i][pattern.getInstIdx()].result == MatchResult.MULTI_FEATURE_MATCH_FAILED) {
						matchReport.result = MatchResult.MULTI_FEATURE_MATCH_FAILED;
						matchReport.reason = String.format("There exists already a fixed index pattern for pattern %d with fix index %d", i, pattern.getInstIdx());
						return matchReport;
					}
					if (pattern.getInstIdx() < instances.size()) {
						...
						if (m.result == MatchResult.MATCH_SUCCESS) {
							...
							matchReport.result = MatchResult.MULTI_FEATURE_MATCH_FAILED;
							matchReport.reason = String.format("No match for pattern %d with fix index %d", i, pattern.getInstIdx());
							return matchReport;
						}
					} else {
						matchReport.result = MatchResult.MULTI_FEATURE_MATCH_FAILED;
						matchReport.reason = String.format("There exists no instance (ArrayOutOufBound) for pattern %d with fix index %d", i, pattern.getInstIdx());
						return matchReport;
					}
				}
			}
		...
		for (int i = 0; i < size; ++i) {
			...
			if (!foundMatch) {
				try {
					matchReport.reason += "No match found for pattern " + EmfPersistence.writeToString(existsPatterns.get(i)) + "\n";
				} catch (Exception ex) {
