...
if "%0"=="-n" goto shift_2
if "%0"=="-d" set _TARGETDIRECTORY_=%1
if "%0"=="-d" goto shift_2
...
:help
echo pax-move-bundle -n bundleName -d targetDirectory [-o] [-- mvnOpts ...]
echo @ http://www.ops4j.org/projects/pax/construct/help/move-bundle.html
...
if ""=="%_TARGETDIRECTORY_%" set PAX_PROMPT=true
...
if ""=="%PAX_PROMPT%" goto skip_input
  ...
  set _INPUT_=
  set /p _INPUT_="targetDirectory (%_TARGETDIRECTORY_%) ? "
  if not ""=="%_INPUT_%" set _TARGETDIRECTORY_=%_INPUT_%

  echo.
:skip_input

@echo on
mvn -N %PAX_PLUGIN%:move-bundle -DbundleName=%_BUNDLENAME_% -DtargetDirectory=%_TARGETDIRECTORY_% %_EXTRA_%
:done
